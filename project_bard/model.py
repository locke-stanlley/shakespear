"""
model.py - Production-Grade T4-Optimized Transformer for Inference
Features:
  ...
  - Optimized, verified Top-P / Top-K / Repetition Penalty sampling
"""
import math
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, rope_cache=None, past_kv=None, use_kv_cache=False):
        B, T, C = x.shape
        ...
        if self.use_flash and T > 1:
            y = F.scaled_dot_product_attention(q, k, v, dropout_p=self.attn_drop.p if self.training else 0.0, is_causal=True)
        else:
        ...
        return self.resid_drop(self.out_proj(y)), new_kv

class Block(nn.Module):
    def __init__(self, n_embd: int, n_head: int, hidden: int, dropout: float, use_rope: bool, use_flash: bool, use_swiglu: bool):
        super().__init__()
    ...
    def forward(self, x, rope_cache, past_kv=None, use_kv_cache=False):
        ...
        return x + attn_out + self.mlp(self.norm2(x)), new_kv
...
class ShakespeareGPT(nn.Module):
    def __init__(self, cfg: ModelConfig = ModelConfig()):
        ...
        self.blocks = nn.ModuleList([
            Block(cfg.n_embd, cfg.n_head, cfg.mlp_hidden, cfg.dropout, cfg.use_rope, cfg.use_flash_attn, cfg.use_swiglu)
        ...
        if cfg.use_rope:
            cos, sin = precompute_rope_cache(cfg.n_embd // cfg.n_head, cfg.block_size, cfg.rope_theta)
    ...
    def _block_forward(self, block, x, rope_cache, past_kv, use_kv_cache):
        return block(x, rope_cache, past_kv, use_kv_cache)

    def forward(self, idx: torch.Tensor, targets: torch.Tensor = None, past_kvs: Optional[list] = None, use_kv_cache: bool = False):
        B, T = idx.shape
        ...
        for i, block in enumerate(self.blocks):
            ...
            if self.cfg.use_grad_checkpoint and self.training and not use_kv_cache:
                x, new_kv = torch.utils.checkpoint.checkpoint(self._block_forward, block, x, rope_cache, past_kv, use_kv_cache, use_reentrant=False)
            else:
                x, new_kv = block(x, rope_cache, past_kv, use_kv_cache)
            if use_kv_cache: new_kvs.append(new_kv)
    ...
    @torch.no_grad()
    def generate(self, idx: torch.Tensor, max_new_tokens: int, temperature: float = 1.0, top_k: int = None, top_p: float = None, repetition_penalty: float = 1.0) -> torch.Tensor:
        """Optimized, batched generation for maximum inference speed."""
        past_kvs = None
        device = idx.device
        generated_ids = idx.clone()
        
        # 1. Prefill phase (process prompt in one go)
